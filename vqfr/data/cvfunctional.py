...
def rotate(img, angle, resample='BILINEAR', expand=False, center=None):
    ...
    if expand:
        if center is None:
            ...
            xx = []
            yy = []
            for point in (np.array([0, 0, 1]), np.array([w - 1, 0, 1]), np.array([w - 1, h - 1,
                                                                                  1]), np.array([0, h - 1, 1])):
                target = M @ point
                xx.append(target[0])
                yy.append(target[1])
...
def perspective(img,
    ...
    r = np.array([[cosy, 0, -siny, 0], [-siny * sinx, cosx, -sinx * cosy, 0], [cosx * siny, sinx, cosx * cosy, 0],
                  [0, 0, 0, 1]])
    ...
    result_img = cv2.warpPerspective(
        img, total_matrix, (w, h), flags=INTER_MODE[resample], borderMode=cv2.BORDER_CONSTANT, borderValue=fillcolor)
    if gray_scale:
